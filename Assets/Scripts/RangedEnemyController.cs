...
public class RangedEnemyController : MonoBehaviour
{
    ...
    IEnumerator Attack(Vector2 target)
    {
        ...
        GameObject proj = Instantiate(projectile, _transform.position, Quaternion.identity);
        var projCont = proj.GetComponent<ProjectileController>();
		projCont.Direction = target;
        yield return new WaitForSeconds(1.5f); // shouldn't move shortly after attack
        state = State.Normal;
    }
